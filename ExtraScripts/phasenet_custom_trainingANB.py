...
def bandpass_filter(data, fs, lowcut=1e5, highcut=1.5e6, order=4):
    nyq = 0.5 * fs
    low = lowcut / nyq
    high = highcut / nyq
    if not (0 < low < high < 1):
...
class SlidingWindowDataset(Dataset):
    def __init__(self, root_dir, label_csv, window_size=50000, stride=20000, gauss_std=200,
                 amp_scale_range=(0.8, 1.2), filter_config=None):
    ...
    def __getitem__(self, idx):
        ...
        # No per-window normalization here (already normalized globally)
...
def train_model(data_dir, label_csv, checkpoint_path, log_csv, window_size, gauss_std,
                epochs=5, batch_size=20, filter_config=None):
...
if __name__ == "__main__":
    ...
    GAUSS_STD = [100]
    FILTER_CONFIGS = [
        {"lowcut": 1e3, "highcut": 1e6, "order": 4},
        {"lowcut": 1e3, "highcut": 1e6, "order": 3},
        {"lowcut": 1e3, "highcut": 1e6, "order": 5}
    ]
    ...
    for w in WINDOWSIZE:
        for g in GAUSS_STD:
            for f in FILTER_CONFIGS:
                fc_name = f"{int(f['lowcut']/1e3)}k{int(f['highcut']/1e3)}k{int(f['order'])}o"
                config = {
                    "checkpoint": f"{w}w{g}gANB_{fc_name}.pt",
                    "log": f"log_{w}w{g}gANB_{fc_name}.csv",
